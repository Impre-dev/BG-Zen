...
| v0.7.6 | **Point flash livré PROPREMENT (doctrine leçon 8)** : revert vers `f9b478e` puis application du seul fix réel — v4 *texture GPU chaude* : couche loading `opacity: 0.01` (jamais `visibility:hidden` : la texture WebRender reste en VRAM entre les sessions) + `warmTexture()` pose la var image à l'idle (après `transitionend` du fade de sortie, guard `exitMs+100`) → activation = pur flip d'opacité. `applyLoading` log CHAUDE/FROIDE. **Aucun** autre changement vs v0.7.4 : v5 abandonné (symptôme fantôme), épluchage différé (point 6 file). Validé user en profondeur : flash absent, zéro régression. | `66e3303` |
